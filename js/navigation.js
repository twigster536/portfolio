...
window.PortfolioNavigation = (() => {
  const sectionOrder = ["profile", "projects", "plc", "entertainment", "motion", "memory", "certificates", "resume", "contact"];
  const sections = {
    ...
    entertainment: { label: "Entertainment", code: "04", description: "Interactive portfolio experiences built around engineering systems and automation storytelling." },
  ...
  let elements;
  const sectionAngles = Object.freeze({ profile: 0, projects: 40, plc: 80, entertainment: 120, motion: 160, memory: 200, contact: 240, resume: 280, certificates: 320 });
  ...
  function renderEntertainment() { const brief = create("section", "entertainment-brief"); const kicker = create("p", "content-kicker", "ENTERTAINMENT MODULE / GRID GUARDIAN"); const title = create("h2", "", "Grid Guardian"); title.id = "contentTitle"; const subtitle = create("p", "entertainment-subtitle", "Interactive 3D Grid Control Room"); const description = create("p", "content-description", "Enter the control room, interfere with the power system, and see how our maintenance robot reacts."); const status = create("div", "entertainment-status"); status.append(create("span", "online-dot"), create("span", "", "FINAL BLENDER ENVIRONMENT READY")); const enter = create("button", "entertainment-enter", "Enter Control Room"); enter.type = "button"; enter.addEventListener("click", () => window.ControlRoom?.open()); brief.append(kicker, title, subtitle, description, status, enter); elements.content.replaceChildren(brief); }
  ...
  function openActiveModule() { if (activeKey === "projects") { openProjectDetail(); return; } if (activeKey === "plc" && window.PlcTimeline) { lastFocused = document.activeElement; window.PlcTimeline.open(); return; } if (activeKey === "resume") { openResumeViewer(); return; } if (activeKey === "memory" && window.MemoryWall) { lastFocused = document.activeElement; window.MemoryWall.open(); return; } if (activeKey === "entertainment") { elements.content.querySelector(".entertainment-enter")?.focus(); return; } if (activeKey === "motion") { window.EngineeringLab?.openLab(elements.openSelected); return; } if (activeKey === "profile" && window.ProfileSystem) { lastFocused = document.activeElement; window.ProfileSystem.open((target) => { closeDetail(); selectSection(target); if (target === "resume") window.setTimeout(openResumeViewer, 60); }); return; } const section = sections[activeKey]; lastFocused = document.activeElement; const title = create("h2", "", section.label); title.id = "modalTitle"; const description = create("p", "", section.description); const note = create("div", "empty-state", "This module is ready for verified portfolio content."); elements.modalContent.replaceChildren(title, description, note); showDetailsModal("PORTFOLIO MODULE"); }
  ...
  function handleKeydown(event) { if (event.key !== "Escape") return; if (window.EngineeringLab?.isOpen()) { window.EngineeringLab.closeLab(); return; } if (elements.searchModal.classList.contains("is-open")) closeSearch(); else if (elements.detailModal.classList.contains("is-open")) closeDetail(); }
  function applyHash() { const hash = location.hash.slice(1); const isLegacyMotionLab = hash === "motion-lab"; if (hash === "engineering-lab" || isLegacyMotionLab) { if (isLegacyMotionLab) history.replaceState({ section: "engineering-lab" }, "", "#engineering-lab"); selectSection("motion", { writeHistory: false, record: false, animate: false }); window.setTimeout(() => window.EngineeringLab?.openLab(undefined, { history: false, directRoute: history.state?.section !== "engineering-lab" }), 0); return; } if (window.EngineeringLab?.isOpen()) window.EngineeringLab.closeLab({ history: false, restoreFocus: false }); if (hash === "home" || !hash) selectSection("projects", { writeHistory: false, record: false, animate: false }); else if (hash === "hmi" || hash === "games") selectSection("entertainment", { writeHistory: false, record: false, animate: false }); else if (hash === "grid-guardian") { selectSection("entertainment", { writeHistory: false, record: false, animate: false }); window.setTimeout(() => window.ControlRoom?.open({ history: false }), 0); } else if (sections[hash]) selectSection(hash, { writeHistory: false, record: false, animate: false }); else if (hash === "recent" || hash === "diagnostics") renderContent(hash); }
