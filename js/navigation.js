...
window.PortfolioNavigation = (() => {
  const sectionOrder = ["profile", "projects", "plc", "hmi", "motion", "memory", "certificates", "resume", "contact"];
  const sections = {
    ...
    hmi: { label: "HMI & SCADA", code: "04", description: "HMI design and testing are listed in the supplied resume. Add verified interface or SCADA case studies here when available.", facts: [["CAPABILITY", "HMI design and testing"], ["APPLICATION", "Control systems"], ["STATUS", "Case studies pending"]] },
  ...
  let elements;
  const sectionAngles = Object.freeze({ profile: 0, projects: 40, plc: 80, hmi: 120, motion: 160, memory: 200, contact: 240, resume: 280, certificates: 320 });
  ...
  function applyHash() { const hash = location.hash.slice(1); if (hash === "home" || !hash) selectSection("projects", { writeHistory: false, record: false, animate: false }); else if (sections[hash]) selectSection(hash, { writeHistory: false, record: false, animate: false }); else if (hash === "recent" || hash === "diagnostics") renderContent(hash); }
