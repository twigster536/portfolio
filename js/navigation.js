...
window.PortfolioNavigation = (() => {
  ...
  function applyHash() { const hash = location.hash.slice(1); if (hash === "home" || !hash) selectSection("projects", { writeHistory: false, record: false, animate: false }); else if (hash === "hmi" || hash === "games") selectSection("entertainment", { writeHistory: false, record: false, animate: false }); else if (hash === "grid-guardian") { selectSection("entertainment", { writeHistory: false, record: false, animate: false }); window.setTimeout(() => window.ControlRoom?.open({ history: false }), 0); } else if (sections[hash]) selectSection(hash, { writeHistory: false, record: false, animate: false }); else if (hash === "recent" || hash === "diagnostics") renderContent(hash); }
